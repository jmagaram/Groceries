...
namespace WebAppTest
{
    ...
    {
        public static T Pushed<T>(this PushResult<T> p) where T : class =>
            p switch
            {
                PushResult<T>.ConcurrencyConflict _ => null,
                PushResult<T>.Pushed res => res.Item2,
                _ => throw new NotImplementedException()
            };

        public static T Conflict<T>(this PushResult<T> p) where T : class =>
            p switch
            {
                PushResult<T>.ConcurrencyConflict c => c.Item,
                PushResult<T>.Pushed _ => null,
                _ => throw new NotImplementedException()
            };
    }
    ...
    public class CosmosConnectorTests
    {
        ...
        public async Task PushCoreAsync_WhenInsert_ReturnWithEtag()
        {
            ...
            Assert.Equal(doc.ClearEtag(), result.Pushed().ClearEtag());
            Assert.True(result.Pushed().HasAnEtag);
        }
        ...
        public async Task PushCoreAsync_WhenUpdate_ReturnWithEtag()
        {
            ...
            var doc2 = (await c.PushCoreAsync(doc1, i => i._eTag, source.Token)).Pushed();
            var doc3 = doc2 with { Title = "banana" };
            var doc4 = (await c.PushCoreAsync(doc3, i => i._eTag, source.Token)).Pushed();
        ...
        public async Task PushCoreAsync_WhenEtagMismatch_ReturnConcurrencyConflict()
        {
            ...
            Assert.Equal(doc4.Conflict(), doc3);
        }
        ...
        public async Task PushBulkCoreAsync_ReturnEachWithUniqueEtag()
        {
            ...
            AssertSameDocuments(docs1, docs2.Select(i => i.Pushed().ClearEtag()));
            Assert.True(docs2.All(i => i.Pushed().HasAnEtag));
        }
        ...
        public async Task PushBulkCoreAsync_WhenSomeConflicts_ReturnSuccessesAndFailures()
        {
            ...
            var docs2 = (await c.PushBulkCoreAsync(docs1, i => i._eTag, source.Token)).Select(i => i.Pushed()).ToImmutableArray();
            ...
            Assert.Equal(2, docs4.Count(i => i.IsConcurrencyConflict));
            Assert.Equal(itemCount - 2, docs4.Count(i => !i.IsConcurrencyConflict));
        }
